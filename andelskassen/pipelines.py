...
class AndelskassenPipeline:

    # Database setup
    conn = sqlite3.connect('andelskassen.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `andelskassen`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM andelskassen WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO andelskassen (date, title, link, content)"
